...
### 2. 上述版本和 https://github.com/andyYuanFZM/chain33_tender_deploy中版本区别是：1. 对交易检查和执行做了一些优化， 2. 使用了聚合签名
...
|----|----|----|----|----|
|用户账户创建|不存在待创建的用户账户|1 发起创建用户账户的请求；2 再次发起创建同一个用户账户的请求|1. 创建成功 2.不能重复创建同名账户|registeAndQueryAccount()|
...
|系统配置信息的有效性检查|系统做好部署的准备工作|1. 部署新资源，设置错误的预设信息  2. 启动新资源; 3. 部署新资源，设置正确的预设信息 4. 启动新资源|1. 无法启动新资源，返回错误提示信息;2. 成功启动新资源，并成功地提供服务|配置错误的共识节点IP，共识无法正常进行|
...
|区块配置修改|系统启动前|1.离线情况下，配置区块的容量；2.在线状态下，修改共享账本的容量限制|在线情况下修改不会生效，同时不影响系统运行；离线情况下修改后再重启，配置生效|手工修改配置测试|
|监控节点状态|系统正常运转|1.监控集群中的节点数目；2.监控节点的运行状态|可以查看p2p节点，共识节点，区块高度等相关信息|可通过rpc或命令行查看|
